...
<head>
 ...
 <meta name="viewport" content="width=device-width, initial-scale=1">
	...
	<link href="{{asset('app/admin/css/animate.css')}}" rel="stylesheet">
	<link href="{{asset('app/admin/css/style.css')}}" rel="stylesheet">
 	<link rel="stylesheet" type="text/css" href="{{asset('app/admin/css/login.css')}}"/>
	<link href="{{asset('app/admin/css/style.css')}}" rel="stylesheet">
	<script type="text/javascript" src="{{asset('plugins/js/jquery-2.1.1.min.js')}}" ></script>
	<link rel="stylesheet" type="text/txt" src="{{asset('app/common/css/bootstrapValidator.css')}}"/>
	<script type="text/javascript" src="{{asset('app/common/js/bootstrapValidator.js')}}"> </script>
...
@forelse($errorsInfo as $errorItem)
	<div class="pnotice" style="display: block;">{{$errorItem}}</div>
@empty
@endforelse
		<div class="middle-box loginscreen animated fadeInDown">
			<div style="background:#fff;margin-top:60px;padding:20px;border-top:3px solid #1dc5a3;">
				...
				<form class="m-t" role="form" id="loginForm" method="post" action="{{ route('Login.postIndex') }}">
					{!! csrf_field() !!}
					<h3 class="tt">
						用户名
					</h3>
